...
# Configurar logging
logging.basicConfig(
    filename=os.path.join('logs', 'youtube_downloader.log'),
    level=logging.INFO,
...
def verificar_tamanho_log():
    """Verifica se o arquivo de log excede 250MB"""
    log_file = os.path.join('logs', 'youtube_downloader.log')
    if os.path.exists(log_file):
        tamanho_mb = os.path.getsize(log_file) / (1024 * 1024)  # Converter para MB
        return tamanho_mb >= 250
    return False

def comprimir_e_rotacionar_log():
    ...
    try:
        ...
        arquivo_7z = os.path.join('logs', f"log_backup_{timestamp}.7z")
        ...
        if os.path.exists(arquivo_7z):
            tamanho_original = os.path.getsize(log_file) / (1024 * 1024)
            ...
            logging.basicConfig(
                ...
                format='%(asctime)s - %(levelname)s - %(message)s',
                encoding='utf-8',
                filemode='w'  # Criar novo arquivo
            )
            
            # Log da rotação no novo arquivo
            log_info(f"Rotação de log realizada: {arquivo_7z}")
            log_info(f"Tamanho original: {tamanho_original:.2f}MB")
            log_info(f"Tamanho comprimido: {tamanho_comprimido:.2f}MB")
            log_info(f"Taxa de compressão: {taxa_compressao:.1f}%")
            
            return True
        else:
            log_error("Falha na compressão do arquivo de log", "Rotação de log")
            return False
            
    except Exception as e:
        log_error(e, "Erro durante rotação de log")
        return False

def limpar_logs_antigos(dias_manter=30):
    ...
    try:
        ...
        arquivos_backup = glob.glob(os.path.join('logs', "log_backup_*.7z"))
        ...
        for arquivo in arquivos_backup:
            try:
                ...
                timestamp_str = arquivo.replace("log_backup_", "").replace(".7z", "")
                data_arquivo = datetime.strptime(timestamp_str, "%Y%m%d_%H%M%S")
                
                if data_arquivo < data_limite:
                    os.remove(arquivo)
                    removidos += 1
                    log_info(f"Arquivo de log antigo removido: {arquivo}")
            except (ValueError, OSError) as e:
                log_error(e, f"Erro ao processar arquivo {arquivo}")
        
        if removidos > 0:
            log_info(f"Limpeza concluída: {removidos} arquivo(s) de log antigo(s) removido(s)")
            
    except Exception as e:
        log_error(e, "Erro durante limpeza de logs antigos")
...
def baixar_video():
    ...
    video_format_id = None # Alterado de format_id para video_format_id

    # Tenta encontrar um stream de vídeo puro para a resolução selecionada primeiro
    if info and 'formats' in info: # Garante que info e info['formats'] estão disponíveis
        for fmt_obj in info['formats']: # Renomeado 'format' para 'fmt_obj' para clareza
            if fmt_obj.get('resolution') == selected_resolution and \
               fmt_obj.get('vcodec') != 'none' and fmt_obj.get('acodec') == 'none':
                video_format_id = fmt_obj['format_id']
                log_info(f"Formato de vídeo puro selecionado: {video_format_id} para resolução {selected_resolution}")
                break
    
    # Se não encontrar um stream de vídeo puro, pega o primeiro stream de vídeo que corresponda à resolução
    if not video_format_id and info and 'formats' in info: # Garante que info e info['formats']
        for fmt_obj in info['formats']: # Renomeado 'format' para 'fmt_obj' para clareza
            if fmt_obj.get('resolution') == selected_resolution and \
               fmt_obj.get('vcodec') != 'none':
                video_format_id = fmt_obj['format_id']
                log_info(f"Formato de vídeo selecionado (com áudio ou áudio não especificado): {video_format_id} para resolução {selected_resolution}")
                break
    
    if not video_format_id:
        messagebox.showerror("Erro", f"Não foi possível encontrar um formato de vídeo adequado para a resolução {selected_resolution}.")
        # Reseta elementos da UI de forma consistente
        baixar_button.config(state=tk.NORMAL, text="Baixar vídeo")
        progress_frame.grid_remove()
        # A variável is_downloading é global para a função baixar_video e já foi definida como True.
        # Precisamos redefini-la aqui em caso de erro antes de iniciar a thread.
        is_downloading = False # Reseta a flag de download
        return
    ...
    def download_worker():
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                # Usar caminho absoluto para o ffmpeg no diretório atual
                ffmpeg_path = os.path.abspath("ffmpeg.exe")
                log_info(f"Usando ffmpeg em: {ffmpeg_path}")
                
                # Verificar se o arquivo existe
                if not os.path.exists(ffmpeg_path):
                    log_error(f"FFMPEG não encontrado em: {ffmpeg_path}", "Configuração FFMPEG")
                    raise FileNotFoundError(f"FFMPEG não encontrado em: {ffmpeg_path}. Verifique se o arquivo está presente no diretório da aplicação.")
            
            # Opções melhoradas para lidar com erros de conexão
            ydl_opts = {
                'format': f"{video_format_id}+bestaudio", # Usa video_format_id
                ...
                # 'postprocessors' foi removido para usar o FFmpegMerger padrão com base em merge_output_format
                'merge_output_format': 'mp4',
                'windowsfilenames': True, # Adicionado para evitar problemas com nomes de arquivo no Windows
                'quiet': False,
                'retries': 10,
                'fragment_retries': 10,
                'skip_unavailable_fragments': True,
                'keep_fragments': False, # Garante que False para limpar arquivos temporários
                'abort_on_unavailable_fragment': False,
                'socket_timeout': 30,
                'http_chunk_size': 10485760,
                # Opções adicionadas:
                'writesubtitles': False,      # Para evitar problemas com legendas/chat
                'writeautomaticsub': False, # Para evitar problemas com legendas/chat
            }
...
def download_success():
    ...
    try:
        ...
        # Verificar e executar rotação de logs se necessário
        if verificar_tamanho_log():
            log_info("Arquivo de log excedeu 250MB, iniciando rotação...")
            if comprimir_e_rotacionar_log():
                # Executar limpeza de logs antigos após rotação bem-sucedida
                limpar_logs_antigos(30)  # Manter logs dos últimos 30 dias
...
def finalize_ui():
    """Finaliza a interface após download"""
    global is_downloading
    is_downloading = False
    baixar_button.config(state=tk.NORMAL, text="Baixar vídeo")
